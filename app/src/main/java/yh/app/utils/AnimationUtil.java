...
public class AnimationUtil {
	public static void slideview(final View view, final float p1, final float p2, final float p3, final float p4,
			long durationMillis, long delayMillis, final boolean startVisible, final boolean endVisible) {
		// 如果处在动画阶段则不允许再次运行动画
	...
	public static void slideButtons(Context context, final FloatingDraftButton button) {
		...
		ArrayList<Button> buttons = button.getButtons();

		if (button.isDraftable()) {// 可拖拽 可以播放动画
			...
			if (buttonLeft >= radius && buttonRight >= radius && buttonTop >= radius && buttonBottom >= radius) {
				...
				for (int i = 0; i < size; i++) {
					Button faButton = buttons.get(i);
					slideview(faButton, 0, radius * (float) Math.cos(Math.toRadians(randomDegree + angle * i)), 0,
							radius * (float) Math.sin(Math.toRadians(randomDegree + angle * i)), 500, 0, true, true);
				}
