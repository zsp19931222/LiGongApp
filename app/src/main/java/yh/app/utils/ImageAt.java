...
public class ImageAt extends AsyncTask<String, Bitmap, Bitmap>
{
	...
	public ImageAt(String imageurl, View image, Context context, String function_name, int defaultImage)
	{
		this.context = context;
		this.image = image;
		this.imageurl = imageurl;
		this.defaultImage = defaultImage;
		ImagePath = ExampleApplication.IMAGE_FUNCTION_PATH;
	}
	...
	protected Bitmap doInBackground(String... params)
	{
		...
		try
		{
			...
			if (FileIsExist(fn))// 图片存在
			{
				try
				{
					tmpBitmap = BitmapFactory.decodeFile(ImagePath + fn);
				} catch (Exception e)
				{
					tmpBitmap = null;
				}
	...
	protected void onPostExecute(Bitmap result)
	{
		...
		image.setBackground(new BitmapDrawable(context.getResources(), result));
	}
