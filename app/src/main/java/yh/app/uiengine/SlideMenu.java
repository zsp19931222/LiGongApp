	...
	public boolean onTouchEvent(MotionEvent ev)
	{
		...
		case MotionEvent.ACTION_MOVE:
			if (mTouchState == TOUCH_STATE_SCROLLING)
			{
				...
				{
					if (!isOpened && home.currentView == 0)
					{

					} else if (!isOpened && home.currentView != 0 && home.currentView < home.getHomeViewCount())
					{
						changedView = -1;
					}
				} else if (deltaX > 0)
				{
					if (!isOpened && home.currentView < home.getHomeViewCount() - 2)
					{
						changedView = 1;
					}
		...
		case MotionEvent.ACTION_UP:
			...
			if (!isOpened)
			{
				...
				home.currentView = msg.what = home.currentView + changedView;
				Constants.viewpagerChange.sendMessage(msg);
			}
			mTouchState = TOUCH_STATE_REST;
			break;
		case MotionEvent.ACTION_CANCEL:
