 ...
 * <p>
 * 分组RecyclerViewAdapter
 */

public class EntityAdapter extends SectionedRecyclerViewAdapter<HeaderHolder, DescHolder, RecyclerView.ViewHolder> {
    ...
    public void setData(MoreEntity allTagList) {
        ...
        allTagList.getAllTagsList().remove(tagInfoListBeen);
...
//        }
    ...
    protected DescHolder onCreateItemViewHolder(ViewGroup parent, int viewType) {
        return new DescHolder(mInflater.inflate(R.layout.item_compile_desc, parent, false));
    }
...
//        holder.openView.setText(mBooleanMap.get(section) ? "关闭" : "展开");
    ...
    protected void onBindItemViewHolder(DescHolder holder, final int section, final int position) {
        ...
        try {
            if (allTagList.getAllTagsList().get(section).getTagInfoList().get(position).isIsSelected()) {
                holder.cutImage1.setBackgroundResource(R.drawable.tj);
            } else {
                holder.cutImage1.setBackgroundResource(R.drawable.ytj);
            }
            holder.descView1.setText(allTagList.getAllTagsList().get(section).getTagInfoList().get(position).getTagName());
            GlideLoadUtils.getInstance().glideLoad(mContext, allTagList.getAllTagsList().get(section).getTagInfoList().get(position).getLatlon(),holder.descImage1,R.drawable.ico_load_little);

        } catch (Exception e) {
            holder.cutImage1.setVisibility(View.INVISIBLE);
            holder.descView1.setVisibility(View.INVISIBLE);
        }
    }
