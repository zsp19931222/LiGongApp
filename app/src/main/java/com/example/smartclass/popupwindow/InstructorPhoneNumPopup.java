...
public class InstructorPhoneNumPopup extends PopupWindow {
    ...
    public InstructorPhoneNumPopup(final Context context, final String name, final String phone_num) {
        conditionList = new ArrayList<>();
        conditionList.add("辅导员：");
        ...
        quickAdapter = new QuickAdapter<String>(conditionList) {
            ...
            public void convert(VH holder, final String data, int position) {
                holder.setTextView(R.id.item_condition_text, data+name+" "+phone_num);
                holder.setRelativeLayout(R.id.item_condition_rel).setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        EventBus.getDefault().post(new MessageEvent("呼叫", phone_num));
                        dismiss();
                    }
                });
            }
