...
public class ContactAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void handleContact(boolean first) {
        ...
        for (int i = 0; i < mContactNames.size(); i++) {
            if (mContactNames.get(i).get("name") != null) {
                ...
                headMap.put(pinyin, mContactNames.get(i).get("handimg"));
        ...
        for (int i = 0; i < mContactList.size(); i++) {
            ...
            try {
                resultList.add(new Contact(map.get(name), ITEM_TYPE.ITEM_TYPE_CONTACT.ordinal(), headMap.get(name), fqrMap.get(name)));
            } catch (Exception e) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof CharacterHolder) {
            ...
            ((ContactHolder) holder).idText.setVisibility(View.VISIBLE);
            ((ContactHolder) holder).idText.setText(resultList.get(position).getFqr());
...
//                        mContext.startActivity(intent);
    ...
    public class ContactHolder extends RecyclerView.ViewHolder {
        ...
        ContactHolder(View view) {
            ...
            idText = view.findViewById(R.id.contact_id);
            imageView = view.findViewById(R.id.contact_image1);
        }
    }
