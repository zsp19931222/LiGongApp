package com.example.smartclass.broadcast_receiver;
...
public class NetworkChange extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (!mobNetInfo.isConnected() && !wifiNetInfo.isConnected()) {
            Log.i("通知", "网络不可以用");
            setChange(none);
            EventBus.getDefault().post(new MessageEvent(TagUtil.NetWorkChanged, HintTool.NoNetWork));
        } else if (mobNetInfo.isConnected()) {
            Log.i("通知", "仅移动网络可用");
            EventBus.getDefault().post(new MessageEvent(TagUtil.NetWorkChanged, HintTool.NetWork));
            setChange(mobile);
        } else if (wifiNetInfo.isConnected()) {
            Log.i("通知", "Wifi网络可用");
            EventBus.getDefault().post(new MessageEvent(TagUtil.NetWorkChanged, HintTool.WiFi));
            setChange(wifi);
        }
    }
}
