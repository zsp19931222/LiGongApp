...
public class SchoolIdentificationActivity extends BaseRecyclerViewActivity {
    ...
    protected void setTitle(Context context) {
        bindingTitle.setTitle(DefaultUtil.getDefaultSchool(), context);
    ...
    protected void init(Context context) {
        ...
        bindingText.setText("学校身份认证");
        bindingTipsText.setText("该账号密码为数字化校园账号密码！");
        bindingNumText.setText("账号");

        bindingNumText2.setText("密码");
        bindingShowText.setText("显示");

        bindingNumEd.setHint("请输入您的学号/教职工号");
    ...
    public void onEventMainThread(MessageEvent event) {
        ...
        if (ActivityUtil.isForeground(this))
            switch (event.getTag()) {
                case TagUtil.verifyIdentificationTag:
                    OtherLoginEntity loginEntity = GsonImpl.get().toObject(result, OtherLoginEntity.class);
                    new SqliteHelper().execSQL("update password4 set password=?", loginEntity.getContent().getPassword());
...
//                OtherLoginEntity loginEntity1 = GsonImpl.get().toObject("{\"content\":{\"password\":\"tDuQi7N+8sxf7FMotv+t4pORFq6M+0//k0P2OO779RvaMF6hQDZwLfqnwfvnMFs9USvNmf0DiOTX\\nxtCr6HBNtEOw/JAyMxoPA7EXu8JHmFSyqqmcnuWJrfDS4p1Z1VnRYuo0a3hG4Op7B2y1XgNW3qWZ\\nnkAgQRt1Yq+kvWUg8UE=\\n\"},\"message\":\"成功\",\"code\":\"40001\"}", OtherLoginEntity.class);
//                new SqliteHelper().execSQL("update password4 set password=?",loginEntity1.getContent().getPassword());
//                UserMessageTool.getPassWord();
                    break;
            }
    }
    ...
    private class MyTextWatcher implements TextWatcher {
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            ...
            if (bindingNumEd2.getText().toString().length() > 0 && bindingNumEd.getText().toString().length() > 0) {
                bindingNextBtn.setBackground(ContextCompat.getDrawable(SchoolIdentificationActivity.this, R.drawable.border_shade_3da8f5));
                canClick = true;
            } else {
                bindingNextBtn.setBackground(ContextCompat.getDrawable(SchoolIdentificationActivity.this, R.drawable.border_shade_e5e9ee));
            }
            //限制输入空格
    ...
    public void onViewClicked(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.binding_next_btn:
                showLoad("");
                new SchoolIdentificationPresenter(this).verifyIdentification(bindingNumEd.getText().toString(), bindingNumEd2.getText().toString());
                break;
