...
public class ContactAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void handleContact(boolean first) {
        ...
        for (int i = 0; i < mContactNames.size(); i++) {
            if (mContactNames.get(i).get("name") != null) {
                ...
                if (mContactNames.get(i).get("name").equals("山西")) {
                    map.put("shangxi", "山西");
                    fqrMap.put("shangxi", mContactNames.get(i).get("FRIEND_ID"));
                    urlMap.put("shangxi", mContactNames.get(i).get("handimg"));
                    mContactList.add("shangxi");
                } else {
                    map.put(pinyin, mContactNames.get(i).get("name"));
                    fqrMap.put(pinyin, mContactNames.get(i).get("FRIEND_ID"));
                    urlMap.put(pinyin, mContactNames.get(i).get("handimg"));
        ...
        for (int i = 0; i < mContactList.size(); i++) {
            ...
            resultList.add(new Contact(map.get(name), ITEM_TYPE.ITEM_TYPE_CONTACT.ordinal(), urlMap.get(name), fqrMap.get(name)));
        }
        if (!first) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof CharacterHolder) {
            ...
            if (resultList.get(position).getmUrl().equals("")) {
                ((ContactHolder) holder).imageView.setVisibility(View.GONE);
                ((ContactHolder) holder).itemView.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        EventBus.getDefault().post(new MessageEvent(TagUtil.ChooseAreaTag, resultList.get(position).getmName()));
                    }
                });
            } else {
                ((ContactHolder) holder).imageView.setVisibility(View.VISIBLE);
                ...
                ((ContactHolder) holder).itemView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (!Tool.isFastDoubleClick()) {
                            ...
                            intent.putExtra("universityName", resultList.get(position).getmName());
